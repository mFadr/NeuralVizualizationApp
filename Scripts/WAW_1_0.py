...
file_path = COM_PATHS['WAW']

try:
    csv_data = pd.read_csv(file_path, header=None, on_bad_lines='skip', engine='python')
    print("CSV file loaded successfully into 'csv_data'!")
    print(csv_data.head())
except FileNotFoundError:
    print(f"Error: The file was not found at '{file_path}'. Please ensure the path is correct and your Drive is mounted.")
    exit(1)
...
column_names = [
    'collection_date', 'flight_date', 'origin', 'dest', 'sched_dep_time',  # 0-4  WAW_com_US
    'sched_duration', 'price', 'airline', 'flight_status',                  # 5-8  WAW_com_US
...
# csv_data is already a DataFrame, so we assign the column names directly.
df = csv_data
...
df['dep_hour'] = pd.to_datetime(df['sched_dep_time'], format='%I:%M %p').dt.hour
...
if __name__ == "__main__":
    ...
    output_file_path = OUTPUT_PATHS['WAW_form']
