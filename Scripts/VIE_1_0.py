...
file_path = COM_PATHS['VIE']

# VIE source rows have variable length:
#   Operated  (14 cols): date,date,origin,dest,dep,dur,price,airline,flight_no,aircraft,act_dur,dep2,act_dep,arr
#   Canceled   (9 cols): date,date,origin,dest,dep,dur,price,no,FLIGHT CANCELED
# Use names= so short rows get NaN for missing trailing columns automatically.
raw_columns = [
    'collection_date', 'flight_date', 'origin', 'dest', 'sched_dep_time',  # 0-4
    'sched_duration', 'price', 'airline', 'flight_no_or_status',           # 5-8
    'aircraft_raw', 'actual_duration', 'sched_dep_time_2',                 # 9-11
    'actual_dep_time', 'sched_arr_time'                                    # 12-13
]

try:
    df = pd.read_csv(file_path, names=raw_columns, header=None,
...
# Derive flight_status and flight_number from the mixed column (index 8)
df['flight_status'] = df['flight_no_or_status'].apply(
    lambda x: 'FLIGHT CANCELED' if 'CANCELED' in str(x).upper() else 'SCHEDULED'
)
df.rename(columns={'flight_no_or_status': 'flight_number'}, inplace=True)

# Extract aircraft code from "E75L (PH-EXH)" → "E75L"
# Canceled rows have NaN here, which is fine
df['aircraft'] = df['aircraft_raw'].astype(str).str.split('(').str[0].str.strip()
df['aircraft'] = df['aircraft'].replace('nan', np.nan)

# airline column: 'no' means canceled — normalise to empty string so emission lookup skips it
df['airline'] = df['airline'].astype(str).str.strip()
df.loc[df['airline'].str.lower() == 'no', 'airline'] = ''

print("DataFrame created with updated column names:")
print(df.head())


print("Čistím data...")

# A) Vyčištění a převod ceny na desetinné číslo (float)
# Remove all non-digit and non-decimal characters before converting to float
df['price'] = pd.to_numeric(df['price'].astype(str).str.replace(r'[^\d.]', '', regex=True), errors='coerce')

# B) Převod datumů na formát datetime
# Check for malformed dates before conversion
invalid_dates = df[~df['collection_date'].str.match(r'^\d{4}-\d{2}-\d{2}$', na=False)]
...
# Optionally, show which rows have NaT after conversion
if df['collection_date'].isna().any():
    print(f"\n{df['collection_date'].isna().sum()} collection dates could not be parsed")

# Remove rows with invalid collection dates
df = df.dropna(subset=['collection_date'])

# Convert flight_date with error handling and filter out invalid dates
df['flight_date'] = pd.to_datetime(df['flight_date'], errors='coerce')

# Check for rows with invalid dates before dropping
invalid_flight_dates = df[df['flight_date'].isna()]
if not invalid_flight_dates.empty:
    print(f"\nFound {len(invalid_flight_dates)} rows with invalid flight_date values:")
    print(invalid_flight_dates[['collection_date', 'flight_date', 'origin', 'dest']].head())

# Remove rows with invalid dates
df = df.dropna(subset=['flight_date'])

# C) Očištění názvu aerolinky (odstranění teček a mezer na koncích)
df['airline'] = df['airline'].astype(str).str.strip(' .')

# D) Vytvoření indikátoru zrušeného letu (Binární hodnota)
...
airline_avg = emision_df.groupby(emision_df['airline'].str.lower())[['fuel', 'co2', 'seats']].mean().to_dict('index')

def get_emision(airline, aircraft):
    if pd.isna(airline) or str(airline).strip() == '':
        return {'fuel': np.nan, 'co2': np.nan, 'seats': np.nan}
    air_clean = str(airline).strip().lower()
    ac_clean  = str(aircraft).strip() if not pd.isna(aircraft) else ''
    if 'wizz'        in air_clean: air_clean = 'wizz air'
    elif 'easyjet'   in air_clean: air_clean = 'easyjet'
    elif 'ryanair'   in air_clean: air_clean = 'ryanair'
    elif 'smartwings' in air_clean: air_clean = 'smartwings'
    key = (air_clean, ac_clean)
    if key in lookup:
        return lookup[key]
    if air_clean in airline_avg:
        return airline_avg[air_clean]
    return {'fuel': np.nan, 'co2': np.nan, 'seats': np.nan}
...
# E) Vyčištění duplicitního sloupce sched_dep_time_2 (můžeme ho rovnou smazat)
...
# Calculate Est. Fuel (kg), Est. CO2 (kg), AVG CO2 (kg/hr)
...
#    NOTE: makes column mixed type (float + string).
...
# Kolik dní předem byla letenka nalezena
df['days_to_departure'] = (df['flight_date'] - df['collection_date']).dt.days

# Den v týdnu pro odlet (jméno dne pro hezčí grafy)
df['day_name'] = df['flight_date'].dt.day_name()
# 0 = Pondělí, 6 = Neděle (pro strojové zpracování)
df['day_of_week'] = df['flight_date'].dt.dayofweek
df['is_weekend'] = df['day_of_week'].apply(lambda x: 1 if x >= 5 else 0)

# Extrakce hodiny odletu (pro zjištění, zda jsou ranní lety levnější)
# Převod '12:00 PM' na časový formát a extrakce hodiny (0-23)
df['dep_hour'] = pd.to_datetime(df['sched_dep_time'], format='mixed', errors='coerce').dt.hour


print("\n--- VÝSLEDKY STATISTICKÝCH TESTŮ ---")

# T-Test: Jsou víkendové lety statisticky významně dražší než lety v týdnu?
weekend_prices = df[df['is_weekend'] == 1]['price']
weekday_prices = df[df['is_weekend'] == 0]['price']

# Provedeme t-test (pokud máme dostatek dat v obou skupinách)
if len(weekend_prices) > 0 and len(weekday_prices) > 0:
    ...
    print(f"P-hodnota (T-test víkend vs. týden): {p_val:.4f}")
    if p_val < 0.05:
        print("-> Závěr: Existuje statisticky významný rozdíl v cenách mezi víkendem a pracovním týdnem.")
    else:
        print("-> Závěr: Nelze prokázat statisticky významný rozdíl (může to být malým vzorkem).")


print("\nGeneruji vizualizace...")

# Nastavení vizuálního stylu pro Seaborn (aby to vypadalo profesionálně)
sns.set_theme(style="whitegrid")

# Vytvoříme plátno s více grafy (2 řádky, 2 sloupce)
fig, axes = plt.subplots(2, 2, figsize=(16, 12))
fig.suptitle('Explorační analýza cen letenek (Trasa: z PRG)', fontsize=18)

# Graf 1: Distribuce cen (Histogram)
...
# Graf 2: Vývoj ceny v závislosti na blížícím se odletu (Zlatý grál tvé práce)
...
axes[0, 1].set_xlabel('Počet dní do odletu (Days to Departure)')
axes[0, 1].set_ylabel('Průměrná cena (USD)')
# Obrátíme osu X, aby čas ubíhal směrem k 0 (den odletu)
axes[0, 1].invert_xaxis()

# Graf 3: Rozptyl cen podle dnů v týdnu (Boxplot)
# Seřadíme dny logicky od pondělí do neděle
days_order = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
sns.boxplot(data=df, x='day_name', y='price', order=days_order, ax=axes[1, 0], hue='day_name', palette='Set2', legend=False)
axes[1, 0].set_title('Volatilita cen v jednotlivých dnech týdne')
axes[1, 0].set_xlabel('Den odletu')
axes[1, 0].set_ylabel('Cena (USD)')
axes[1, 0].tick_params(axis='x', rotation=45)

# Graf 4: Vztah mezi časem odletu (hodinou) a cenou
sns.barplot(data=df, x='dep_hour', y='price', ax=axes[1, 1], hue='dep_hour', palette='viridis', errorbar=None, legend=False)
...
# Function to get unique origins from dataset in order of first appearance
def get_origin_filter_options():
    """
    Extracts unique origin airports from the dataset in order of first appearance.
    Returns a list of origin airport codes (e.g., ['BUD', 'PRG', 'BER'])
    """
    # Get unique origins while preserving order of first appearance
    unique_origins = df['origin'].dropna().unique().tolist()
    return unique_origins


# Function to transform data for chartActive.py
def get_csv_string_for_dash():
    """
    Transforms the BER dataframe into CSV string format for chartActive.py
    Returns formatted CSV string with columns: search_date, flight_date, origin, destination, departure_time, duration, price, airline_details
    """
    ...
    # Rename columns to match chartActive.py format
    ...
    # Clean airline_details: convert NaN to empty string and ensure all are strings
    ...
    # Convert to CSV string with semicolon delimiter
    csv_string = export_df.to_csv(index=False, sep=';')

    return csv_string


# Print first 10 rows of the transformed data
if __name__ == "__main__":
    # Display available origin filter options
    print("\n--- ORIGIN FILTER OPTIONS ---")
    origin_options = get_origin_filter_options()
    print(f"Available origins (in order of first appearance): {origin_options}")

    csv_output = get_csv_string_for_dash()

    from config import OUTPUT_PATHS
    output_file_path = OUTPUT_PATHS['VIE_form']
